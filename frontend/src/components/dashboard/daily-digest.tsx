...
import { Post, CATEGORY_COLORS, CONSENSUS_COLORS } from '@/types/post';
...
export function DailyDigest({ posts, onPostClick }: DailyDigestProps) {
  const digest = useMemo(() => {
    ...
    return { insights, digestPosts, period, topTags };
  }, [posts]);

  if (!digest || digest.insights.length === 0) return null;
      ...
      {digest.topTags.length > 0 && (
        <div className="mt-4 pt-3 border-t border-border">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-xs text-muted-foreground">Sujets récurrents :</span>
